...
require('lazy').setup({
	...
	-- jsonnet and libsonnet
	'google/vim-jsonnet',
	-- obsidian / markdown (hope it doesn't conflict with vim-markdown)
	{
		"epwalsh/obsidian.nvim",
		version = "*",
		lazy = true,
		-- See if this is specific enough. Might want to only use it
		-- when inside vault folders, but I don't edit that much
		-- markdown anyway
		ft = "markdown",
		dependencies = {
			-- Required
		...
		config = function()
			require('obsidian').setup({
				workspaces = {
					{
						name = "personal",
						path = "~/Documents/obsidian-test/default_vault/",
					},
					{
						name = "work",
				...
				-- TODO: Figure out if the settings are taken from the obsidian JSON or I need to always specify them here... would be nice to unify
				-- They are not, sadly
				daily_notes = {
					folder = "dailies",
					date_format = "%Y-%m-%d",
					-- If I want tags, I'll add them via the template, thanks
					default_tags = {},
					template = "zz_templates/Daily note",
				},
				...
				ui = {
					checkboxes = {
						-- Replace with this if you don't have a patched font:
			...
			-- TODO: Add additional binds probably based on which commands I end up using
			vim.keymap.set('n', '<leader>oo', ':ObsidianOpen')
			vim.keymap.set('n', '<leader>oq', ':ObsidianQuickSwitch')
			vim.keymap.set('n', '<leader>ob', ':ObsidianBacklinks')
			-- Obsidian Daily Daily (today)
			vim.keymap.set('n', '<leader>odd', ':ObsidianToday')
			-- Obsidian Daily Tomorrow (workweek)
			vim.keymap.set('n', '<leader>odt', ':ObsidianTomorrow')
			-- Obsidian Daily Yesterday (workweek)
			vim.keymap.set('n', '<leader>ody', ':ObsidianYesterday')
		end
	}
})
